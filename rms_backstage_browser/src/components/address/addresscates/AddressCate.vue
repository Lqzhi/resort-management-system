...
export default {
  ...
  methods: {
    //获取最新的数据
    async getUsersMes() {
      const { data } = await this.$http.get('room/type')
      console.log(data);
      if (data.status !== 200)
    ...
    //添加事件
    async commit(mes) {
      const {data} = await this.$http.post('room/type', mes)
    ...
    async reGet(userInfo, index) {
      ...
      const { data } = await this.$http.delete(
        'room/type',{data:{id :userInfo[index].id}}
      )
      console.log(data);
    ...
    async editUserMes(userMes) {
      console.log(userMes);
      const { data } = await this.$http.put(`room/type`,userMes)
      console.log(data);
