<template>
  <div>
    ...
    <users_addDialog :isShow="isShow" @isFalse="isFalse" @commit="commit" :rolesList = "rolesList" />
  </div>
</template>
...
export default {
  ...
  methods: {
    //获取最新的数据
    async getUsersMes() {
      const { data } = await this.$http.get('admin/find', {
    ...
    async change(userInfo) {
      console.log(userInfo.admin_state);
      const {data} = await this.$http.put(
        `admin/updatestate`,{id:userInfo.id,admin_state:userInfo.admin_state}
      )
      if (data.status !== 200) {
        //更新失败后要保证状态未被改变
        userInfo.admin_state = !userInfo.admin_state
    ...
    //添加用户事件
    async commit(mes) {
      const {data} = await this.$http.post('admin/add', mes)
    ...
    async reGet(userInfo, index) {
      ...
      const { data } = await this.$http.delete(
        'admin/del',{data:{id :userInfo[index].id}}
      )
      console.log(data);
    ...
    async editUserMes(userMes) {
      console.log(userMes);
      const { data } = await this.$http.put(`admin/update`,userMes)
      console.log(data);
